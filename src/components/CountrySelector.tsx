    ...
    <div className="glass-card p-4">
      ...
      <div className="relative">
        <select
          ...
          className="w-full bg-[var(--bg-secondary)] border border-[var(--border)] rounded-xl px-4 py-3 pr-10 text-sm font-medium appearance-none cursor-pointer hover:border-amber-400 focus:border-amber-600 focus:outline-none transition-colors text-[var(--text-primary)]"
        >
          {markets.map((market) => (
      ...
      {selected && (
        <div className="mt-3 grid grid-cols-3 gap-2 text-xs">
          ...
          <div className="bg-gray-50 rounded-lg p-2">
            ...
            <p className="font-bold text-blue-700">{selected.centralBankRate}%</p>
          </div>
        </div>
      )}
    </div>
